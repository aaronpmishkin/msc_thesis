...
However, in this work we assume that backtracking can be accomplished exactly, meaning that \( \etak \) is the largest step-size less than \( \etamax \) for which \autoref{eq:deterministic-armijo-ls} is satisfied.
...
\begin{table}[t]
    ...
    \begin{tabular}{c l l c c }\toprule
        ...
        \cmidrule(lr){2-3} \cmidrule(lr){4-5}
                 & \multicolumn{1}{c}{Ours} & \multicolumn{1}{c}{VML \citep{vaswani2019painless}}%
                 & \multicolumn{1}{c}{Ours} & \multicolumn{1}{c}{VML \citep{vaswani2019painless}}\\ \midrule
    \( \mu \)-SC & \( c \geq \half \)%
                 & \( c \geq \half \)%
    ...
    Convex       & \( c \geq \half \)%
        ...
        \end{tabular}
        \caption{Comparison of iteration complexities for \ac{SGD} with the stochastic Armijo line-search.  We omit results for non-convex functions, which are identical between the two works. Results are shown for \( \etamax = \infty \) and \( c = \frac{1}{2} \), excepting the convex case of VML (Vaswani-Mishkin-Laradji~\citep{vaswani2019painless}) where \( c = \frac{2}{3} \) is used as suggested by the authors. The constant \( \bar \mu = \min_k \cbr{\E_{\zk} \sbr{\mu_{zk}} } \) requires that at least one stochastic function in the support of \( \zk \) is strongly convex for each iteration \( k \); in contrast, our strongly-convex proof relies only on the parameter of the true function \( \mu \).  }%
