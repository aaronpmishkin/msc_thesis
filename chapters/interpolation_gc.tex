...
\todo{Note somewhere that requiring a condition point-wise over \( \calZ \) is equivalent to requiring it holds almost surely for all \( k \), since every outcome in \( \calZ \) as is in the support of some \( \mu_k \). }
...
\end{restatable} 
A simple sufficient condition for the equivalence of maximal strong and strong growth is that the support set \( \calZ \) for the SFO is finite.
