...
%% Actually, it's not obvious how to extend interpolation-type results to projected/proximal-gradient methods, since the minimizer no longer satisfies interpolation.
...
\end{restatable}
See \autoref{app:stochastic-oracles} for proof. \hfill \break
...
We call general SFOs \oracle{} individually-smooth specifically to invite comparison with this natural property in the finite-sum setting. 
...
For general functions and SFOs, the relationship between these models of interpolation is limited to the following: minimizer interpolation and stationary-point interpolation are stronger than mixed interpolation. 
...
\begin{restatable}[Interpolation and Strong Growth]{lemma}{interpToSGC}~\label{lemma:interpolation_to_sgc}
    Let \( f \) be an \( L \)-smooth function and \oracle{} an \( \Lmax \) individually-smooth SFO.
    Furthermore, assume that \( \rbr{f, \oracle{}} \) satisfies interpolation.
...
\end{restatable}
Note that the Polyak-Łojasiewicz implies \( f \) is invex and thus minimizer, stationary-point, and mixed interpolation are equivalent conditions, which we simply refer to as interpolation.
This convention will be continued in the following chapters.
