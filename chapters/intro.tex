...
\end{gather*}
We will focus on interpolation conditions of this form, which we extend to general objective functions and stochastic gradients.\\ 
}
...
\edit{Suppose \( f \) is finite-sum and \( P(i) \) is an arbitrary sub-sampling distribution used to compute stochastic gradients. 
    Then strong growth with parameter \( \rho > 0 \) may be written as 
\[ \text{Maximal Strong Growth:} \quad \norm{\grad_i(\w)}^2 \leq \rho \norm{\grad(\w)}^2, \hspace{10.5em} \]
where the inequality holds almost-surely with respect to \( P \).}
...
\edit{\[ \text{Strong Growth:} \quad \E_P \sbr{\norm{\grad_i(\w)}^2} \leq \rho \norm{\grad(\w)}^2. \hspace{8.6em} \]}
...
\edit{\[ \text{Weak Growth}: \quad \E\sbr{\norm{\grad_i(\w)}^2} \leq 2 \rho L \rbr{f(\w) - f(\wopt)}. \hspace{4.2em} \]}
...
\edit{\[ \text{Strong Growth + Noise:} \quad \E_P \sbr{\norm{\grad_i(\w)}^2} \leq \rho \norm{\grad(\w)}^2 + \sigma^2, \hspace{10em} \]}
...
\edit{\citet{honorio2012biased} consider accelerated proximal-gradient methods under biased stochastic oracles and show high-probability convergence is not guaranteed due to accumulated errors.} 
...
For finite-sum functions, variance reduction techniques can be combined with Nesterov acceleration to improve convergence on the stochastic component~\citep{allen-zhu2017katyusha, allen-zhou2018katyushax, tang2018restkatyusha, kovalev2020loopless}.
