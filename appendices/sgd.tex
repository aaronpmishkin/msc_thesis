...
\begin{proof}
   ...
   \implies \E \sbr{f(\wkk)} - f(\wopt) &\leq \rbr{\frac{L}{\mu}} \rbr{1 - \frac{2 \eta \mu L}{\mu + L}}^k \rbr{f(\w_0) - f(\wopt)}.
    \end{align*}
\end{proof}
...
\begin{proof}
   \begin{align*}
       ...
       \implies \E\sbr{\wkk - \wopt}^2 &\leq \rbr{1 - \mu \, \eta \rbr{2 - \eta \Lmax}}^k \norm{\w_0 - \wopt}^2.
       \intertext{\autoref{lemma:iterate-bounds} now completes the proof: }
       \implies \E\sbr{f(\wkk)} - f(\wopt) &\leq \rbr{\frac{L}{\mu}} \rbr{1 - \mu \, \eta \rbr{2 - \eta \Lmax}}^k\rbr{f(\w_0) - f(\wopt)}. 
   \end{align*}
\end{proof}
...
\begin{proof}
    \begin{align*}
       ...
       \intertext{Application of \autoref{lemma:iterate-bounds} now completes the proof,}
   \implies f(\wkk) - f(\wopt) &\leq \rbr{\frac{L}{\mu}} \rbr{1 - 2 \eta \, \delta}^k \rbr{f(\w_0) - f(\wopt)}.
    \end{align*}
\end{proof}
...
\begin{lemma}\label{lemma:wgc-fast-extended}
   ...
   \[ \E \sbr{f(\wkk)} - f(\wopt) \leq \rbr{\frac{L}{\mu}} \rbr{1 - \mu \, \eta + \max\cbr{\eta \mu \rbr{\eta \alpha L - 1}, \eta L \rbr{\eta \alpha L - 1}}}^k \rbr{f(\w_0) - f(\wopt)}. \]
   Moreover, this is tight with the original result when \( \eta = \frac{1}{\alpha L} \).
\end{lemma}
\begin{proof}
    Starting from the seventh line of the proof of Theorem 5 in Appendix B.4 of \citet{vaswani2019fast},
   \begin{align*}
       ...
       \intertext{Taking expectations, recursing on the inequality, and then applying \autoref{lemma:iterate-bounds} gives the final rate,}
        \implies \E \sbr{\norm{\wkk - \wopt}^2} &\leq \rbr{1 - \mu \, \eta + \max\cbr{\eta \mu \rbr{\eta \alpha L - 1}, \eta L \rbr{\eta \alpha L - 1}}}^k \norm{\w_0 - \wopt}^2\\
        \implies \E \sbr{f(\wkk)} - f(\wopt)  &\leq \rbr{\frac{L}{\mu}} \rbr{1 - \mu \, \eta + \max\cbr{\eta \mu \rbr{\eta \alpha L - 1}, \eta L \rbr{\eta \alpha L - 1}}}^k \rbr{f(\w_0) - f(\wopt)}.
   \end{align*}
